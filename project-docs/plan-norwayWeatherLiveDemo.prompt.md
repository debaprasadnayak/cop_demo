...
```bash
databricks sql statement-execution execute-statement --profile dbx_free --warehouse-id 0de9375ba014b54e --statement "<same-sql>"
```
...
Use fixed `warehouse_id`: `0de9375ba014b54e` for all `mcp_databricks_execute_sql`,
...
```bash
databricks dashboards get "$DASHBOARD_ID" --profile dbx_free
```
...
```bash
databricks genie-spaces get "$SPACE_ID" --profile dbx_free
```
