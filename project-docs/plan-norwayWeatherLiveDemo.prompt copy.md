...
description: >
  ...
  Agent B (support-analytics-reporter): creates the AI/BI Dashboard and
...
| **support-analytics-reporter** | Phase B: create AI/BI Dashboard + Genie Space, run live Q&A smoke-test |
...
def main():
    ...
    for c in cities:
        ...
        insert_row(w, wh.id, WEATHER_TABLE, run_ts, c["city"], c["lat"], c["lon"], url, payload)
        print(f"  ✓ {c['city']}")
    ...
    for c in cities:
        ...
        insert_row(w, wh.id, AIR_TABLE, run_ts, c["city"], c["lat"], c["lon"], url, payload)
        print(f"  ✓ {c['city']}")
...
## A2 — Update Bundle Config and Deploy

### A2.1 — Update `databricks.yml`
...
targets:
  dev:
    ...
    default: true
    workspace:
      host: https://dbc-b74acda9-4dbe.cloud.databricks.com
```

### A2.2 — Validate bundle
...
## A3 — Create Unity Catalog Objects
...
### A3.3 — Reference cities table + data (idempotent)
...
After pipeline completes, verify layer counts with `execute_sql`:
...
## ════════════════════════════════════════════════════════
## PHASE B  ·  support-analytics-reporter
## ════════════════════════════════════════════════════════

> Invoke the **support-analytics-reporter** agent for all steps in Phase B.
...
Use `get_table_details` MCP tool for:
...
Use `get_best_warehouse` MCP tool. Note the warehouse ID.

### B1.3 — Validate all dataset queries (mandatory — do not skip)

Test with `execute_sql`. Fix any failure before building the dashboard.

**Dataset `ds_sunshine_rank`** (bar chart):

```sql
SELECT city, ROUND(sunshine_hours_14d, 1) AS sunshine_hours_14d, rank
FROM   cop_weather_demo.gold.sunshine_14d_rank
WHERE  run_ts = (SELECT MAX(run_ts) FROM cop_weather_demo.gold.sunshine_14d_rank)
ORDER  BY rank ASC
```
...
**Dataset `ds_clean_air`** (air quality comparison):
...
Use `create_or_update_dashboard` MCP tool:
...
- **3 widgets**:
  ...
  - **Visual 3** — Table
    - Dataset: `ds_clean_air`  |  Columns: `rank_pm25`, `rank_aqi`, `city`, `avg_pm25_ug_m3`, `avg_european_aqi`
    - Title: `Air Quality Comparison – 7-Day Horizon`
- **Filters**: `run_ts` parameter (mandatory, all datasets); `city` multi-select (optional)

### B1.5 — Publish dashboard

```python
publish_dashboard(dashboard_id="{id}", publish=True)
```
...
> - Visual 3: PM2.5 + AQI table for all 8 cities ✓
>
...
Use `create_or_update_genie` MCP tool with the following parameters verbatim.
...
### B2.1 — Smoke-test with `ask_genie`

Use `ask_genie` MCP tool with the `space_id`. Ask all 4 seed questions.
Verify:
...
Ask these questions in the Genie Space browser UI (or via `ask_genie`):
...
| Step | Est. Time | Agent |
|------|-----------|-------|
| A0: Auth pre-flight | <30 s | Senior Data Engineer – Databricks |
| A1: Create 7 source files | ~2 min | Senior Data Engineer – Databricks |
| A2: Bundle validate + deploy | ~1 min | Senior Data Engineer – Databricks |
| A3: UC catalog/schemas/tables | ~1 min | Senior Data Engineer – Databricks |
| Gate 1 | <30 s | Presenter |
| A4: Bronze ingestion (16 API calls) | ~1 min | Senior Data Engineer – Databricks |
| Gate 2 | <30 s | Presenter |
| A5: SDP pipeline run (4 MVs) | ~90 sec | Senior Data Engineer – Databricks |
| Gate 3 | <30 s | Presenter |
| B1: Dashboard (3 queries + build) | ~2 min | support-analytics-reporter |
| Gate 4 | <30 s | Presenter |
| B2: Genie Space + smoke-test | ~1.5 min | support-analytics-reporter |
| Gate 5 | <30 s | Presenter |
| B3: Live Q&A | ~1–2 min | Presenter |
| **Total** | **~12 min** | |

---

> **Post-demo cleanup**: run the commented-out cells at the bottom of
> `cop_demo/notebooks/00_demo_driver.py` (Options 1–4 in order).
